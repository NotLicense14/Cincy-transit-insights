...
/**
 * Project a lat/lon onto a polyline and return the cumulative distance at the
 * closest point. Kept here because it is geometry, not train-specific logic.
 */
function snapToLine(lat, lon, linePoints, cumDist) {
  ...
  for (let i = 0; i < linePoints.length - 1; i++) {
    ...
    let t = 0;
    if (lenSq > 0) {
      t = Math.max(0, Math.min(1, ((lon - ax) * dx + (lat - ay) * dy) / lenSq));
    }
...
/** Return the lat/lon at a cumulative distance along a polyline. */
